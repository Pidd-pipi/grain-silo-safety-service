...
func newOpsAudit() *OpsAudit { return &OpsAudit{events: []OpsEvent{}, byType: map[string]int{}} }
func (a *OpsAudit) Add(recordID, typ, actor string) OpsEvent {
	a.mu.Lock()
	defer a.mu.Unlock()
	event := OpsEvent{ID: newOpsAuditID(), RecordID: recordID, Type: typ, Actor: actor, At: time.Now().UTC().Format(time.RFC3339Nano), Details: map[string]string{"by": actor}}
	a.byType[typ]++
...
func (a *OpsAudit) ByType(typ string) int {
	a.mu.RLock()
	defer a.mu.RUnlock()
	return a.byType[typ]
}
